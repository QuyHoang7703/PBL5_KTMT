...
async function fetchHistories() {
    try {
        ...
        const response = await fetch(`${baseUrl}/history-management/histories/customer/${customerId}`);
        if (!response.ok) {
...
document.getElementById('confirm_btn').addEventListener('click', async () => {
    ...
    try {
        ...
        const response = await fetch(`${baseUrl}/history-management/histories/customer_filler/${customerId}?${query.toString()}`);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const data = await response.json();
