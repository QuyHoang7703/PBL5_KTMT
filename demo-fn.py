...
binary = imutils.resize(binary, width=200)
# Sử dụng morphological closing để lấp đầy các lỗ hổng
...
cv2.waitKey()
_, binary_traditional = cv2.threshold(img_gray, 127, 255, cv2.THRESH_BINARY_INV+ cv2.THRESH_OTSU)
cv2.imshow("binary traditional", binary_traditional)
cv2.waitKey()
