async function fetchTickets() {
    try {
        const response = await fetch('http://10.10.58.56:5000/ticket-management/tickets');
...
async function createTicket(event) {
    ...
    try {
        const response = await fetch('http://10.10.58.56:5000/ticket-management/ticket', {
            method: 'POST',
            headers: {
...
async function viewTicketDetails(ticketId) {
    try {
        const response = await fetch(`http://10.10.58.56:5000/ticket-management/ticket/${ticketId}`);
        if (!response.ok) {
...
async function handleTicketAction(event, formElement, action) {
    ...
    let method = 'PUT';
    let url = `http://10.10.58.56:5000/ticket-management/ticket/${data.vehicle_plate}`;
