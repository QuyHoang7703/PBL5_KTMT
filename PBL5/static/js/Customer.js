...
function populateCustomerTable(customers) {
    ...
    customers.forEach(customer => {
        ...
        row.setAttribute('data-customer-id', customer.id_customer);
        row.innerHTML = `
            ...
            <td>${customer.cccd}</td>
        `;
        row.addEventListener('click', () => viewCustomerDetails(customer.id_customer));
...
async function viewCustomerDetails(customerId) {
    try {
        const response = await fetch(`${baseUrl}/customer-management/customer/${customerId}`);
        if (!response.ok) {
            throw new Error('Network response was not ok');
        }
        const customer = await response.json();
        document.querySelector('input[name="id_view"]').value = customer.id_customer;
...
async function handleAction(event, formElement, actione) {
    ...
    let method = 'PUT';
    let url = `${baseUrl}/customer-management/customer/${data.id_customer}`;
    ...
    try {
        ...
        console.error(`Error ${actione} customer:`, error);
    }
}