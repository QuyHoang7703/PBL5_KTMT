...
def send_to_server(data):
    try:
        response = requests.post("http://localhost:8000/api/receive_send", json=data)  # URL của server Flask
...
# Tập hợp để lưu trữ các biển số xe đã gửi
...
def process_camera_stream(stream_url, model, my_model):
    global last_detection_time, detection_result, detection_confirmed, previous_license_plate, detected_license_plates
    while True:
        try:
                        ...
                        for label in range(1, num_labels):
                            ...
                            if contours:
                                for contour in contours:
                                    ...
                                    if 0.22 < aspect_ratio < 1.0 and 1.0 > solidity > 0.3 and 0.2 < height_ratio < 1.0:
                                ...
                                print("Contours not found for label", label)
                        if candidates:
                            ...
                            print("Không có ứng viên nào được tìm thấy.")
                        current_time = time.time()
                        if previous_license_plate not in detected_license_plates:
                            detected_license_plates.add(previous_license_plate)
                            if (current_time - last_detection_time) >= confirmation_time:
                                # Gửi dữ liệu lên server
                                data_to_send = {
                                    "license_plate": previous_license_plate,
                                }
                            ...
                            print("Biển số đã được gửi trước đó.")
